...
function updateHeroTiles(progress){
  // TEMP: new video's content/segments unknown — split evenly into thirds for now.
  // Tell me what each third of the new video actually shows and I'll adjust these.
  let active;
  if(progress < 0.333) active = 'bathroom';
  else if(progress < 0.666) active = 'office';
  else active = 'kitchen';
...
(function heroAnimation(){
  ...
  const TOTAL_FRAMES = 984; // new hero video, replacing the bath/office/kitchen sequence
  const frames = [];
  let loadedCount = 0;
